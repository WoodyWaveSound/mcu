...
static void debug(const wws_debug_t *dbg)
{
  if (dbg->type == WWS_DEBUG_MESSAGE) {
    printf("[%s][%s] %s", dbg->component, dbg->event, (const char *) dbg->data);
  }
  else {
...
{
  wws_infoln("<%s> [%s] (%d) %s", wws_cli_cmd_type_str[type], cmd->cmd.str, len, ptr);
...
static wws_cli_cmd_t i2c  = {
   ...
   .callback = cmd_callback,
   .arg_num  = 1,
   .children = { &scan, &i2ca, &i2cw, &i2cr, 0 },
};

static wws_cli_t cli = { .io = &io, .root = { .children = { &i2c, 0 } } };
